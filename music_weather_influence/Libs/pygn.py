...
# Custom search method to return only 1 level of genre in attribute
def search_custom(clientID='', userID='', artist='', album='', track='', toc=''):
	"""
	Queries the Gracenote service for a track, album, artist, or TOC
	...
	if (toc != ''):
		...
		query.addQueryTextField('TRACK_TITLE', track)
	query.addQueryOption('SELECT_EXTENDED', 'COVER,REVIEW,ARTIST_BIOGRAPHY,ARTIST_IMAGE,ARTIST_OET,MOOD,TEMPO')
	query.addQueryOption('SELECT_DETAIL', 'GENRE:DEFAULT,MOOD:DEFAULT,TEMPO:DEFAULT,ARTIST_ORIGIN:4LEVEL,ARTIST_ERA:2LEVEL,ARTIST_TYPE:2LEVEL')
